...
(function () {
  ...
  function customizeSidebar(root) {
    ...
    if (sidenavContent && !document.getElementById("tcl-sidebar-logo")) {
      ...
      logoDiv.innerHTML =
        '<img src="/SOGo.woa/WebServerResources/assets/logo.svg" alt="Teclat">';
    ...
    if (isMail || isCalendar || isContacts) {
      var mainSidebar = document.querySelector(
        "md-sidenav.md-sidenav-left md-content",
      );
      ...
      if (
        mainSidebar &&
        logoElement &&
        !document.getElementById("tcl-fake-compose")
      ) {
        ...
        fakeBtn.innerHTML =
          '<md-icon class="material-icons" style="margin-right: 8px;">' +
          iconName +
          '</md-icon><span class="sg-fab-label">' +
          labelText +
          "</span>";
        ...
        fakeBtn.addEventListener("click", function (e) {
          ...
          if (isMail) {
            ...
            var composeBtn = document.querySelector(
              '[ng-click^="mailbox.newMessage"]',
            );
          ...
          window.forceSogoClick = function (actionType) {
            ...
            if (actionType === "card") {
              btn = document.querySelector(
                '[ng-click*="newCard"], [ui-sref*="newCard"], md-fab-actions button:nth-child(1)',
              );
            } else if (actionType === "list") {
              btn = document.querySelector(
                '[ng-click*="newList"], [ui-sref*="newList"], md-fab-actions button:nth-child(2)',
              );
            }
            ...
            if (btn) {
              ...
              if (window.angular) {
                ...
                if (scope) {
                  ...
                  if (scope.app) {
                    if (actionType === "card" && scope.app.newCard)
                      scope.app.newCard();
                    else if (actionType === "list" && scope.app.newList)
                      scope.app.newList();
                  }
                }
              }
        ...
        if (isCalendar && !document.getElementById("tcl-calendar-popup")) {
          ...
          document.addEventListener("click", function (evt) {
            if (
              !evt.target.closest("#tcl-fake-compose") &&
              !evt.target.closest("#tcl-calendar-popup")
            ) {
        ...
        if (isContacts && !document.getElementById("tcl-contacts-popup")) {
          ...
          document.addEventListener("click", function (evt) {
            if (
              !evt.target.closest("#tcl-fake-compose") &&
              !evt.target.closest("#tcl-contacts-popup")
            ) {
      ...
      if (isContacts && mainSidebar) {
        var addIcons = mainSidebar.querySelectorAll(
          ".sg-md-subheader--with-secondary-icon button md-icon",
        );
      ...
      for (var i = 0; i < fabs.length; i++) {
        if (isMail && fabs[i].tagName.toLowerCase() !== "md-fab-speed-dial") {
          ...
          (isCalendar || isContacts) &&
          fabs[i].tagName.toLowerCase() === "md-fab-speed-dial"
        ) {
          fabs[i].style.opacity = "0";
          fabs[i].style.position = "absolute";
          fabs[i].style.zIndex = "-9999";
    ...
    var listItems = sidenavScope.querySelectorAll(
    ...
    for (var k = 0; k < listItems.length; k++) {
      ...
      if (text.includes("entrada")) item.classList.add("tcl-item-entrada");
        ...
        item.classList.add("tcl-item-rascunho");
      ...
      else if (text.includes("arquivado") || text.includes("archive"))
        item.classList.add("tcl-item-arquivado");
      else if (text.includes("modelo")) item.classList.add("tcl-item-modelo");
      else if (text.includes("lixo eletrônico") || text.includes("spam"))
        item.classList.add("tcl-item-lixo");
      else if (text === "menu" || text.includes("aplicativos"))
        item.classList.add("tcl-item-menu");
      else if (text.includes("configuraç"))
        item.classList.add("tcl-item-config");
      else if (text.includes("sair")) item.classList.add("tcl-item-sair");

      // Identifica o header de conta (email sem avatar) e esconde
      if (
        text.includes("@") &&
        !item.querySelector(".md-avatar") &&
        !item.querySelector("sg-avatar") &&
        !item.classList.contains("tcl-item-user")
      ) {
    ...
    var sidenavContents = document.querySelectorAll(
      "md-sidenav.md-sidenav-left md-content",
    );
    sidenavContents.forEach(function (sidenavContent) {
      if (!sidenavContent.querySelector(".tcl-custom-footer")) {
        ...
        if (!userEmail) {
          var accountNode =
            document.querySelector(".tcl-hide-account .sg-no-wrap") ||
            document.querySelector("[ng-bind='::account.name']") ||
            document.querySelector("[ng-bind='::app.user.login']");
          if (accountNode) userEmail = accountNode.textContent.trim();
        }
        if (userName === "Usuário" || userName === userEmail.split("@")[0]) {
          var nameNode =
            document.querySelector(
              ".md-toolbar-tools span[ng-bind='app.user.name']",
            ) || document.querySelector("[ng-bind='::app.user.name']");
        ...
        var avatarSrc =
          ...
          "&background=00cfb0&color=fff";
        var isMailPage =
          (window.location.pathname + window.location.hash).indexOf("/Mail") !==
          -1;
        ...
        footer.innerHTML =
          ...
          <div class="tcl-footer-menu">
            ...
            <div class="tcl-footer-item" onclick="window.toggleTeclatDarkMode()">
              <md-icon class="material-icons tcl-darkmode-icon">` +
          (document.body.classList.contains("tcl-dark-mode")
            ? "wb_sunny"
            : "brightness_2") +
          `</md-icon>
              <span class="tcl-darkmode-text">` +
          (document.body.classList.contains("tcl-dark-mode")
            ? "Modo Claro"
            : "Modo Escuro") +
          `</span>
          ...
          popupHtml +
          `
            <img class="tcl-user-avatar" src="` +
          avatarSrc +
          `" alt="Avatar" />
            <div class="tcl-user-info">
              <div class="tcl-user-name">` +
          userName +
          `</div>
              <div class="tcl-user-email">` +
          userEmail +
          `</div>
            </div>
            ` +
  ...
  function applyCustomAvatars() {
    ...
    document
      .querySelectorAll("md-list-item.sg-message-list-item")
      .forEach((item) => {
        const nameSpan = item.querySelector(
          ".sg-md-subhead > div > span:not(.sg-label-outline)",
        );
  ...
  function applyViewerAvatar(node) {
    ...
    root
      .querySelectorAll(
        ".sg-face md-card-content > .layout-wrap > div:first-child:not(.tcl-viewer-avatar-applied)",
      )
      .forEach((item) => {
        ...
        if (nameSpan && avatarContainer) {
          ...
          if (wrapContainer) {
            const msgRecipients =
              wrapContainer.querySelector(".msg-recipients");
  ...
  function applyComposeAvatar(node) {
    ...
    root
      .querySelectorAll(
        'sg-avatar-image[sg-email="editor.message.editable.from"]',
      )
      .forEach((avatarContainer) => {
        ...
        if (rawText) {
          ...
          if (namePart === extractedEmail || !namePart) {
            name =
              angularUserName || angularUserEmail.split("@")[0] || "Usuário";
          } else {
            name = namePart;
          }
        ...
        if (!tclImg) {
          ...
          const nativeImg = avatarContainer.querySelector(
            "img:not(.tcl-custom-avatar)",
          );
          if (nativeImg) {
  ...
  function customizeCalendarHeader(root) {
    var isCalendar = /\/Calendar\b/.test(
    ...
    if (
      !cardActions &&
      root.tagName &&
      root.tagName.toLowerCase() === "md-card-actions"
    ) {
      cardActions = root;
    }
    ...
    if (
      cardActions &&
      !document.querySelector(".tcl-calendar-date-container")
    ) {
      ...
      var customWeekday = document.createElement("p");
      customWeekday.className = "sg-day";
      var customMonth = document.createElement("p");
      customMonth.className = "sg-month";
      var customYear = document.createElement("p");
      customYear.className = "sg-year";
  ...
  function syncCalendarDate() {
    ...
    var match = hash.match(
      /\/calendar\/(?:day|week|month|multicolumnday)\/(\d{4})(\d{2})(\d{2})/,
    );
    var d = null;
    ...
    if (!d || isNaN(d.getTime())) {
      var datePicker = document.querySelector(
        'md-datepicker[ng-model="calendar.selectedDate"]',
      );
    ...
    if (cWeekday && cWeekday.textContent !== weekdayStr)
      cWeekday.textContent = weekdayStr;
    if (cMonth && cMonth.textContent !== monthStr)
      cMonth.textContent = monthStr;
...
(function forceInlineCompose() {
    ...
    if (window.angular) {
      ...
      if (appEl) {
        ...
        if (
          ...
          scope.app &&
          scope.app.user &&
          scope.app.user.settings &&
          scope.app.user.settings.Mail
        ) {
  ...
  function setupEventModal(dialog) {
    ...
    var interval = setInterval(function () {
      ...
      if (eventForm) {
        ...
        if (section1) {
          var labels = section1.querySelectorAll(
  ...
  function setupEventModalDates(section2) {
    ...
    var allDayCheckbox = section2.querySelector(
      'md-checkbox[ng-model="event.isAllDay"]',
    );
    if (allDayCheckbox) {
    ...
    var isFreeCheckbox = section2.querySelector(
      'md-checkbox[ng-model="event.isFree"]',
    );
    if (isFreeCheckbox) {
      var labelSpan = isFreeCheckbox.querySelector(".md-label");
      if (
        labelSpan &&
        (labelSpan.textContent.trim() === "Mostrar como livre" ||
          labelSpan.textContent.trim() === "Exibir como livre")
      ) {
    ...
    if (
      deDateInput &&
      paraDateInput &&
      deDateInput.value !== paraDateInput.value
    ) {
    ...
    mutations.forEach(function (mutation) {
      mutation.addedNodes.forEach(function (node) {
        if (node.nodeType === 1 && node.tagName.toLowerCase() === "md-dialog") {
          ...
          if (
            node.querySelector('form[name="messageForm"]') ||
            node.querySelector("#dialogContent_mailEditor")
          ) {
            node.classList.add("tcl-mail-modal");
            node.classList.remove("md-dialog-fullscreen"); // Remove fullscreen nativo do angular
          } else if (
            node.querySelector('form[name="eventForm"]') ||
            node.querySelector('form[name="taskForm"]') ||
            node.querySelector('md-icon[aria-label="assignment_turned_in"]')
          ) {
          ...
          node.nodeType === 1 &&
          node.querySelector &&
          node.querySelector("md-dialog")
        ) {
          var dialogs = node.querySelectorAll("md-dialog");
          dialogs.forEach(function (dialog) {
            if (
              dialog.querySelector('form[name="messageForm"]') ||
              dialog.querySelector("#dialogContent_mailEditor")
            ) {
              dialog.classList.add("tcl-mail-modal");
              dialog.classList.remove("md-dialog-fullscreen");
            } else if (
              dialog.querySelector('form[name="eventForm"]') ||
              dialog.querySelector('form[name="taskForm"]') ||
              dialog.querySelector('md-icon[aria-label="assignment_turned_in"]')
            ) {
...
(function () {
  function injectPreferencesSections() {
    ...
    var idiomaRow = container.querySelector(
      'div[layout="row"]:not([layout-align])',
    );
    var modulo = container.querySelector(
      '#select_107, md-select[aria-label*="padrão" i], md-select[aria-label*="implicit" i], md-select[aria-label*="Modul" i], md-select[aria-label*="Módulo" i]',
    );
    var gravatarRow = container.querySelector(
      'div[layout-align="start start"], div[layout="row"][layout-align]',
    );
...
(function () {
  function injectUniversalSaveButton() {
    ...
    Array.from(targetNodes).forEach(function (content, index) {
      ...
      if (
        content.tagName.toLowerCase() === "md-content" &&
        content.classList.contains("ng-hide") &&
        content.querySelector("md-card")
      )
        return;
      ...
      var container =
        content.querySelector('[role="tabpanel"]:not(md-tab-content)') ||
        content.querySelector(
          'md-content:not([ng-include]):not([role="listbox"])',
        ) ||
        content;
      ...
      btn.addEventListener("click", function (e) {
        ...
        var originalBtn = document.querySelector(
          'md-toolbar.md-tall button.md-fab[ng-click*="save"]',
        );
      ...
      function syncDisabledState() {
        ...
        try {
          ...
          var scope =
            window.angular && formElement
              ? window.angular.element(formElement).scope()
              : null;
          if (scope && scope.preferencesForm) {
            ...
            var originalBtn = document.querySelector(
              'button.md-fab[ng-click*="save"]',
            );
            if (originalBtn) {
              isDisabled =
                originalBtn.classList.contains("ng-hide") ||
          ...
          if (originalBtn) {
            isDisabled =
              originalBtn.classList.contains("ng-hide") || originalBtn.disabled;
          }
        }
  ...
  var prefSaveObserver = new MutationObserver(function () {
    if (window.location.href.indexOf("Preferences") === -1) return;
    var hasContent =
      document.querySelector("md-tab-content > div[md-tabs-template]") ||
      document.querySelector('div[ui-view="module"] > md-content');
    if (hasContent) {
...
(function () {
  var isContacts = /\/Contacts\b/.test(
    window.location.pathname + window.location.hash,
  );
  ...
  function injectContactFooter() {
    ...
    var btnCancel = document.querySelector(
      '#detailView button[aria-label="Cancelar"]',
    );
    var btnReset = document.querySelector(
      '#detailView button[aria-label="Limpar"]',
    );
    var btnSave = document.querySelector(
      '#detailView button[aria-label="Salvar"]',
    );
  ...
  contactFormObserver.observe(document.body, {
    childList: true,
    subtree: true,
  });
...
(function () {
  function injectContactViewFeatures() {
    ...
    var addressContainers = detailView.querySelectorAll(
      '.msg-body > div[ng-show="editor.card.addresses"] .pseudo-input-container',
    );
    ...
    addressContainers.forEach(function (container) {
      ...
      var typeLabel = address.type
        ? address.type.charAt(0).toUpperCase() + address.type.slice(1)
        : "";
      var addressTitle = typeLabel
        ? "Endereço (" + typeLabel + ")"
        : "Endereço";
  ...
  document.addEventListener(
    ...
    function (e) {
      ...
      var btn = e.target.closest(
        'button[ng-click*="selectCard"], md-list-item',
      );
      if (btn) {
